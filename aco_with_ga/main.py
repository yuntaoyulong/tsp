...
from distance import calculateDistance, distance_calculation
...
if __name__ == "__main__":
    ...
    for i in range(len(dis_mtx)):
        for j in range(len(dis_mtx)):
            dis_mtx[i, j] = distance_calculation(i, j, dist_matrix1)
    ...
    for i in range(100):
        best, shortest_dis_lst = ACO.acoRound(rounds,tau_mtx,rho,ant_number,city_number, eta_mtx,alpha,beta,dis_mtx,Q,population,lowwer,upper,dim,mut_way,epochs,save,k,CR)  # 进化算法
        best1=np.append(best1,min(shortest_dis_lst))
        if min(shortest_dis_lst) == np.min(best1):
            bestpath = best
        if min(shortest_dis_lst) <2380000:
            break
    # print("最优解：x=", best)
    ...
    # data_df = pd.DataFrame(location, columns=['latitude','longitude']) 
